...
import { updateInstructor } from '../../actions/skillsAction';
...
export default function EditInstructor({ show, close }) {
	const classes = useStyles();
	const dispatch = useDispatch();
	const { instructor } = useSelector((state) => state.skills.selectedskill);
	...
	const setinitialfields = (instructor) => {
		setname(instructor?.name);
		setqualification(instructor?.qualification);
		setdesignation(instructor?.designation);
		setcompanyName(instructor?.companyName);
		settotalExperience(instructor?.totalExperience);
		setdomainExperience(instructor?.domainExperience);
		setskills(instructor?.skills);
		setprojects(instructor?.projects);
	};
	useEffect(() => {
		setinitialfields(instructor);
	}, [instructor]);
